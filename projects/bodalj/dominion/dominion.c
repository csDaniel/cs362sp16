...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonusa) {
    ...
    switch(card) {
        case adventurer:
            return playAdventurer(currentPlayer, handPos, state);
...
int playAdventurer(int currentPlayer, int handPos, struct gameState *state) {
    int TREASURES_TO_DRAW = 2;  // number of cards this card draws
    ...
    int shuffleCount = 0;
    int *p_handcount = &(state->handCount[currentPlayer]);
    ...
    while(z-1 >= 1) {
        ...
        state->discard[currentPlayer][state->discardCount[currentPlayer]-1] = tempHand[z-1]; 	
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag) {
    // if trashFlag == 1 then the card is trash
    
    // current card
    int *played_card = &(state->hand[currentPlayer][handPos]);

    //if card is not trashed, added to Played pile
    if (trashFlag < 1) {
        //add card to played pile
        state->playedCards[state->playedCardCount] = *played_card; 
        state->playedCardCount++;
    }
    ...
    *played_card = -1;
    ...
    //last card in hand array is played
