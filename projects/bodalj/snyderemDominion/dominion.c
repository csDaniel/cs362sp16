...
// jensbodal test helper functions
...
int playBaron(int currentPlayer, int choice1, struct gameState *state) {
    return baronEffect(currentPlayer, state, choice1);
}
int playCouncilRoom(int currentPlayer, int handPos, struct gameState *state) {
    int bonus = -1;
    return cardEffect(council_room, -1, -1, -1, state, handPos, &bonus);
}
...
int playGreatHall(int currentPlayer, int handPos, struct gameState *state) {
    int bonus = -1;
    return cardEffect(great_hall, -1, -1, -1, state, handPos, &bonus);
}
