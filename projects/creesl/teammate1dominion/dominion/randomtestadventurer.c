...
int main()
{
	...
	if (oracle < 0)
	{
		...
		int i;
		for (i = 0; i < testGame.deckCount[whoseTurn(&testGame)]; i++)
		{
			if (testGame.deck[whoseTurn(&testGame)][i] == copper)
				printf("Location: %d, Coin: copper\n", i);
			else if (testGame.deck[whoseTurn(&testGame)][i] == silver)
				printf("Location: %d, Coin: silver\n", i);
			else if (testGame.deck[whoseTurn(&testGame)][i] == gold)
