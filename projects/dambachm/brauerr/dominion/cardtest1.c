...
int main() {
	...
	if (G.handCount[player] != 7) {
		printf("Test Failed! Hand count should be 7 but is %i\n", G.handCount[player]);
		fail = 1;
	}
	...
	if (G.discardCount[player] != 0) {
		printf("Test Failed! Discard count should be 0 but is %i\n", G.discardCount[player]);
		fail = 1;
	}
	if (G.deckCount[player] != 3) {
		printf("Test Failed! Deck count should be 3 but is %i\n", G.deckCount[player]);
		fail = 1;
	}
	if (fail == 0) {
		printf("Test Passed!!\n");
