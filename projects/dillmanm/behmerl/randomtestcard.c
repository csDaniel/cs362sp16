...
void runGreatHallTest(struct gameState*,int);
...
void initKingdomCards(int * kingArray){
	int i, j, randomTemp, redo, hallIndex;
	for(i=0;i<10;i++){
		do {
	...
	//randomly add great hall to a random index in kingdom array
	hallIndex = (int)(Random()*9);
	kingArray[hallIndex] = great_hall;
}
...
void runGreatHallTest(struct gameState * state, int kingCards){
	int i, handPos, handSize, canDraw, expectedAction, playedCardExpected, totalCards, postTotal;
	int cannotDraw = 0;
	int treasureCheck = 0;
	int foundTreasure = 0;
	int discarded = 0;
	...
	if ((handPos = getHandPos(state,player,great_hall))<0) {
		printf("Error: Invalid player or no great hall in current player hand\n\n");
	}
	if (state->deckCount[player] == 0 && state->discardCount[player] == 0){
		cannotDraw = 1;
	}
	//hand size is a scratch, cycle one out unless you cannot draw, in which case 1 is subtracted
	handSize = init.handCount[player]-cannotDraw;
	...
	//action points conserved
	expectedAction = init.numActions;
	//play card
	...
	if (expectedAction == state->numActions) printf("%s: ",success);
	else printf("%s: ",failure);
	printf("Expected post-play actions: %d, Actual: %d.\n",totalCards,postTotal);
	//expected action test
...
void zeroDeck(struct gameState * state){
	//zero the discard and the deck of the active player
	int player = state->whoseTurn;
...
int main(int argc, char** argv){
	...
	SelectStream(0);
	PutSeed(-1);
	printf("=====GREAT HALL TRIALS=====\n");
	for (i=0;i<TRIALS;i++){
		memset(kingArray,0,sizeof(kingArray));
		memset(state,0,sizeof(struct gameState));
		initKingdomCards(kingArray);
		constructGamestate(state,kingArray);
		//forces an empty deck and discard trial
		if (i==(TRIALS-1)){
			zeroDeck(state);
			runGreatHallTest(state,kingArray);
		}
		else runGreatHallTest(state,kingArray);
	}
	return 0;
}