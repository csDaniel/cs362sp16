...
int main() {
    ...
    if (case1Return >= 0) {
        printf("cardEffect() - smithy: PASS good return value\n");
    } else {
        printf("cardEffect() - smithy: FAIL return value indicates something went wrong\n");
    }
    //make sure current player gets 2 new cards (draws 3, discards 1)
    if (testState.handCount[currentPlayer] == case1HandCount + 2) {
        printf("cardEffect() - smithy: PASS two new cards in hand\n");
    } else {
        printf("cardEffect() - smithy: FAIL two new cards not in hand\n");
    }
