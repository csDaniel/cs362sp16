...
int main() {
    ...
    if (case1Return >= 0) {
        printf("cardEffect() - village: PASS good return value\n");
    } else {
        printf("cardEffect() - village: FAIL return value indicates something went wrong\n");
    }
    ...
    if (testState.handCount[currentPlayer] == case1HandCount) {
        printf("cardEffect() - village: PASS gained new card while discarding card\n");
    } else {
        printf("cardEffect() - village: FAIL did not gain new card while discarding card\n");
    }
    //make sure current player got 2 more actions
    if (testState.numActions == (case1NumActions + 2)) {
        printf("cardEffect() - village: PASS incremented number of actions\n");
    } else {
        printf("cardEffect() - village: FAIL did not increment number of actions\n");
    }
