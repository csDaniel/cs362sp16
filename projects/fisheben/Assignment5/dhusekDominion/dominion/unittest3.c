		...
		for (i = 1; i <= ACTION_CARD_TOTAL; i++){
			...
			for(trash = 0; trash < 2; ++trash){
				...
				for(cardPosition = 0; cardPosition < i; ++cardPosition){
					...
					if(postCall.discardCount[p] != (preCall.discardCount[p]+1) && !trash && !errorFlag1){ 
                    ...
                    preCall.playedCardCount++;

					if(memcmp(&preCall, &postCall, sizeof(struct gameState)) != 0 && !trash && ! errorFlag5){ 
