...
void setupDeck(struct gameState * state, int numPlayers, int *kingdomCards){
	int i, j;
...
void smithyRandomTest(const int VERBOSE_TEST){
	struct gameState game, compareGame;
	int k[10];
	int sample = 0; 
	int i, j, curPlayer, handPos, result, problems, x;
	...
	for(i = 0; i < 10000; i++){
		...
		setupDeck(&game, game.numPlayers, k);
		fillDeck(&game, k);
		...
		game.hand[curPlayer][handPos] = smithy;

		//result = smithyPlay(curPlayer, handPos, &game);
		result = smithyPlay(&game, handPos, curPlayer);

		if(compareGame.numPlayers > 0){	
			compareGame.hand[curPlayer][handPos] = smithy;
