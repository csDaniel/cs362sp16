...
Michael Hueter
CS 362 Assignment 4
randomtestcard.c - fuzzing the village card
**/
#include "dominion.h"
...
int main() {
  /************ initial set up ************/
  int villageStatus; // did village exit with 1 or 0
  ...
  k[0] = village; // ensure village is always present
  int numberOfPlayers;
  int seed;
  struct gameState G;
  ...
  for (i = 0; i < NUM_TESTS; i++){
    ...
    for (x = 2; x < 10; x++){
      randomIndex = (rand() % 26); // exclude village
    ...
    G.hand[0][0] = village; // Player 1 always needs an village
    ...
    printf("\n RESULTS: \n");
    /************ run tests ************/
    villageStatus = playVillageCard(&G, 0);
    executionFailures += testAssert(0, villageStatus, "Village card executes and returns 0");
    handSizeFailures += testAssert(handSize, G.handCount[0], "Player 1 hand size");
    actionFailures += testAssert(3, G.numActions, "Player 1 should now have 3 actions");
  ...
  printf ("\n------- RANDOM TESTING Village Card COMPLETE -------\n");
  totalFailures = executionFailures + handSizeFailures + actionFailures + player2Failures + discardFailures;
  if(totalFailures == 0){
