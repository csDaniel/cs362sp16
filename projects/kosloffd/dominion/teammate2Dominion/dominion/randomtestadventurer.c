...
int putAdventurerInHand(struct gameState* game);
void testAdventurer(struct gameState* game, int handIndex, struct errorCounter* ecount, int seedNumber);
...
int main(int argc, char **argv)
{
    ...
    for(i=0; i < numOfRuns; i++)
    {
        ...
        int indexInHand = putAdventurerInHand(game);
        testAdventurer(game, indexInHand, ecount, seed);
        free(game);
        testCounter++;
    }
...
void printErrorTotals(struct errorCounter* ecount, int qtyOfTests)
{
    int i = 0;
    float percent;
    printf("\nError:%-74s Number of Times Encountered:\t\tPercentage of total run times:\n", "");
    while(ecount->errors[i] != NULL)
    {
        ...
        if(percent < 10)
            ...
            printf("\n%-80s\t\t%d\t\t\t\t\t%2.2f%%", ecount->errors[i]->errorName, ecount->errors[i]->qty, percent);
        i++;
    }
}
...
void getRandomHand(struct gameState* game, int* kCards, int player)
{
    ...
    int handSize = rand() % 15 + 1;     // It's not common to have many more than 10 cards in your hand, but you must have at least one to hold adventurer
...
/**
 * Replaces a random card in the player's hand with the adventurer card so the player can play it.
 * params:
 *      game:   The current game state
 * returns:
 *      index:  The index of the adventurer card in the player's hand
 */
int putAdventurerInHand(struct gameState* game)
{
    int player, index;
    ...
    game->hand[player][index] = adventurer;
    return index;
}

/**
 * Plays the adventurer card and compares the game state tot the state before it was played to determine where errors exist.
 * Any errors found are recorded in the errorCounter to be analyzed after the testing is concluded.
 * params:
 *      game:       The current state of the game
 *      handIndex:  The position of the adventurer card in the player's hand
...
void testAdventurer(struct gameState* game, int handIndex, struct errorCounter* ecount, int seedNumber)
{
    ...
    if( initialState->numBuys != game->numBuys)
        addError(ecount, "Changed the number of Buy phases available", seedNumber );
    ...
    if( initialState->handCount[otherPlayer] != game->handCount[otherPlayer])
        addError(ecount, "Changed the number of cards in the other player's hand", seedNumber );
    ...
    if(initialState->deckCount[otherPlayer] != game->deckCount[otherPlayer])
        addError(ecount, "Changed the deck count for the other player", seedNumber );
    ...
    if( (game->hand[thisPlayer][game->handCount[thisPlayer]-1] != gold &&
         game->hand[thisPlayer][game->handCount[thisPlayer]-1] != silver &&
         game->hand[thisPlayer][game->handCount[thisPlayer]-1] != copper) ||
       (game->hand[thisPlayer][game->handCount[thisPlayer]-2] != gold &&
        game->hand[thisPlayer][game->handCount[thisPlayer]-2] != silver &&
        game->hand[thisPlayer][game->handCount[thisPlayer]-2] != copper) )
        addError(ecount, "Did not put the two drawn treasure cards at the end of the player's hand", seedNumber );
