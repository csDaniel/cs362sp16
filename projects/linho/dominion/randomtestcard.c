...
int main() {
	...
	for (i = 1; i <= PLAYNUM; i++){
		...
		// give card
		testGame.hand[thisPlayer][testGame.handCount[thisPlayer]] = village;
		...
		printf("Count = %d, expected = %d\n", testGame.deckCount[thisPlayer], game.deckCount[thisPlayer] + 1);
		if(testGame.deckCount[thisPlayer] - 1 == game.deckCount[thisPlayer])
        {
        	printf("......PASSED\n");
        	passed++;
        }
		...
		printf("Checking for +2 action......\n");
		printf("Count = %d, expected = %d\n", testGame.numActions, game.numActions + 2);
		if(testGame.numActions - 2 == game.numActions)
        {
        	printf("......PASSED\n");
        	passed++;
        }
		...
		printf("Count = %d, expected = %d\n", testGame.handCount[thisPlayer], game.handCount[thisPlayer]);
		if(testGame.handCount[thisPlayer] == game.handCount[thisPlayer])
        {
        	printf("......PASSED\n");
        	passed++;
        }
