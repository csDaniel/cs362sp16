...
(Bug found due to excessive running time of randomtestadventurer.c random test; the test had to be aborted.)
*** Bug Report 0 ***
Title: cardEffect() applies multiple different card effects for a single card.
File: dominion.c
Classification: Severe (game rules violated, unpredictable behavior)
Description: Multiple card effects appear to occur when calling cardEffect() to test the effect of a single card, when called for adventurer, council_room, mine, remodel, and smithy cards. 
Steps to Reproduce:
...
4. The expectation is for a single card to be played, whichever was chosen.
5. Depending on which card is played, multiple card effects will be executed. This manifests itself as multiple cards being played, and accumulation of multiple card effects (e.g. cards added to hand).
Debugging/Fix:
...
(Bug found based on failures produced by unitttest3.c, test of scoreFor() function.)
*** Bug Report 2 ***
Title: Incorrect scoring calculated by scoreFor() function.
File: dominion.c
...
4. The expected score is -1 for a single curse card.
5. The score calculated by scoreFor() is 0.
Debugging/Fix:
...
(Bug found based on failures produced by cardtest3.c, test for council_room card effect)
*** Bug Report 3 ***
...
4. The council_room should cause the player to draw 4 cards.
5. The council_room actually causes the player to draw 5 cards.
Debugging/Fix:
...
(Bug found based on failures in randomtestadventurer.c)
*** Bug Report 4 ***
Title: Adventurer card is not put in played pile when cardEffect() is called for the adventurer card.
File: dominion.c
Classification: Severe (game rules violated)
...
4. An adventurer should end up in the played pile after the cardEffect() function called.
5. There is no change to the played card pile after calling cardEffect() for the adventurer card.
Debugging/Fix:
...
(Bug found based on failures in cardtest1.c, for the smithy card effect)
*** Bug Report 2 ***
Title: Smithy card is not put in played pile when cardEffect() is called for the smithy card.
File: dominion.c
Classification: Severe (game rules violated)
...
4. An smithy should end up in the played pile after the cardEffect() function called.
5. There is no change to the played card pile after calling cardEffect() for the smithy card.
Debugging/Fix:
...
(Bug found based on failures produced by randomtestcard.c, which also tests the smithy card effect)
*** Bug Report 3 ***
...
3. call cardEffect(), playing the smithy card from any handpos.
4. The smithy should cause 3 cards to be removed from the deck.
5. The smithy actually causes 4 cards to be removed from the deck.
Debugging/Fix:
...
(Bug found based on failures in cardtest2.c and on randomtestadventurer.c)
*** Bug Report 4 ***
Title: Adventurer card is not put in played pile when cardEffect() is called for the adventurer card.
File: dominion.c
Classification: Severe (game rules violated)
...
(Bug found based on failures in cardtest2.c)
*** Bug Report 5 ***
Title: Adventurer card effect behavior unpredictable (buffer overflow) if no treasures are available to draw.
File: dominion.c
...
3. call cardEffect(), playing the adventurer card from any handpos.
4. Since no treasures are available, the adventurer card effect should iterate over all cards in the player's deck + discard and ultimately cause no cards to be added to the player's hand or be removed from the deck.
5. An unpredictable (very large) number of cards are removed from the player's hand, deck, and discard.
Debugging/Fix:
