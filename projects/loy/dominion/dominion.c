...
if(card == smithy)
{
for (i = 0; i < 3; i++) //changed back to 3
	{
...
if(card == adventurer)
{
while(drawntreasure<2){  // changed back to < 2
...
{
for (i = 0; i < 4; i++) // changed back to 4
	{
	  drawCard(currentPlayer, state);
	}
...
for (i = 0; i <= state->handCount[currentPlayer]; i++){
	...
	state->hand[currentPlayer][i] = -1;//Set to nothing
      ...
      updateCoins(currentPlayer, state, 5); // changed back to 5
