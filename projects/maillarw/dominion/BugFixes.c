...
The following is a list of bugs found and fixed using the results of my teammates tests, my tests, and GDB.  
The coverage in my tests helped immensely with testing of these bugs.  All of the coverage information can be found in the combinedRandomUnitSaved.out file.  Because the coverage of the functions in question, for the majority of these bugs was 100%, it none of the bugs listed by teammates were not already found by my tests (except one, which was in function that I did not test).  The bugs reported by my teammates and myself in bugs1 have all been fixed and the program should run much better now.  
...
Reason: fullDeckCount() returns the number of a specific card (in this case card 0: curse) in the deck, the hand, and the discard.  The gardens score is instead based on the sum of all cards in all three files, so this was fixed with the totalCards sum.
	
Fix 1.2: In the third loop (counting the score for the deck) this line:
	 for (i = 0; i < state->discardCount[player]; i++) 
was replaced by this line:
	 for (i = 0; i < state->deckCount[player]; i++) 
Reason: This loop was only counting up to the number of discard cards, when it should be counting up to the number of deck cards.
...
Bug #2
Effect: playSmithy() function is not having the desired effect (3 cards are added to hand, 3 cards are removed from the deck)
...
Reason: The i variable in these lines was uninitiallized, causing unpredictable behavior (i could start as any number)
...
Bug #3
//Effect: playAdventurer is not having the desire effect (up 2 two treasure cards (depending on the number of treasure cards in the players deck and discard) are added to the player's hand)
...
Reason: z is a counter that keeps track of how many cards are in the temp hand (holds cards while looking for treasure cards).  z is decremented while cards are discarded from the temp hand.  It was originally decremented by 3 for every card when it should be decremented by only one.
...
Reason: The playAdventurer() function does not discard the adventurer card after it is played.  Because much of the code relies on this function only having 1 parameter, this fix was added to discard an adventurer card without knowing its location.
...
Effect: playVillage() is not having the desired effect (1 card is added to the players and and 2 actions are also added)
...
Effect: playFeast() is not having the desired effect(Trash feast card and gain a card costing up to 5)
...
Reason: This allows testing to see if the game is over with 5 players, as the rules for ending the game require that 4 stacks are empty or the provinces are gone.
