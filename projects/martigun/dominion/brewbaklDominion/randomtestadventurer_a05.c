...
int compareStates(struct gameState *state1, struct gameState *state2){
	
	int i, j;
	int statesMatch = 1; //initialize match to 1
...
int checkAdventurer(int player_nbr, struct gameState *post, int test_run, int handPos) {
	...
	//since they could all be affected
	...
	//test for shuffle
	if(discardChange<0 && deckTreasures_count >= 2){
		
		if (TRACERS) printf("## THERE WAS A SHUFFLE ##: %d\n", discardChange);
		if (TRACERS) printf("## discardChange: %d\n", discardChange);
		if (TRACERS) printf("## treasures in deck: %d\n", deckTreasures_count);
		
		printf("There should not be a shuffle with 2+ treasures in deck.\n");
		printf("test run %d: failed\n", test_run);
	}
	
	//check states and report result
	int statesMatch = compareStates(&pre, post);
	if(statesMatch){
		printf("test run %d: Success\n", test_run);
	} else {
		printf("test run %d: Failed\n", test_run);
