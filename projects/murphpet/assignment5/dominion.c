...
int scoreFor (int player, struct gameState *state) {
  ...
  for (i = 0; i < state->handCount[player]; i++)
    ...
    //  if (state->hand[player][i] == gardens) { score = score + ( fullDeckCount(player, 0, state) / 10 ); }; // old bugged version, fulldeckcount called with bad parameters same bug with 2 below gardens cards
  ...
  //score from deck
  for (i = 0; i < state->deckCount[player]; i++) //fixed bug was discardCount instead of deck
    {
  ...
  int currentPlayer = whoseTurn(state);
   //+4 Cards
    ...
    for (i = 0; i < 4; i++)
	{
      ...
      //+1 Buy
      state->numBuys = state->numBuys + 1; //fixed bug was state->numBuys = state->numBuys +  state->numBuys
...
int i;
 //+3 Cards
      for (i = 0; i < 3; i++)
	{
      ...
      discardCard(handPos, currentPlayer, state, 1 ); //fixed bug was -> currentPlayer + 1
      return 0;
}
