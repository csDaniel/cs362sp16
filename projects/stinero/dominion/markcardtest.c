...
int main() {
	...
	printf("TEST 5: Test for 0 tresure cards in deck\n");
	initializeGame(numPlayers, k, seed, &G);
	memcpy(&testG, &G, sizeof(struct gameState));
	...
	if(testG.handCount[thisPlayer] == G.handCount[thisPlayer] - 1) {
        ...
        printf("FAILURE\n");
        printf("handcount is %d, should be %d", testG.handCount[thisPlayer], G.handCount[thisPlayer]-1);

    }

	// Test 6: verify that if there is one treasure card in the player's deck, one card gets added.
	printf("TEST 6: Test for 1 treasure card in deck");
	initializeGame(numPlayers, k, seed, &G);
...
//	int i;
	// Play adventurer
	cardEffect(adventurer, 0, 0, 0, &testG, handpos, 0);
	if(testG.handCount[thisPlayer] == G.handCount[thisPlayer] && testG.hand[thisPlayer][0] == gold) {
        printf("SUCCESS\n");
    }
    else {
	...
	printf("TEST 7: Test that the adventurer card is removed");
	initializeGame(numPlayers, k, seed, &G);
	memcpy(&testG, &G, sizeof(struct gameState));
