...
void testWhereCardsComeFrom(struct gameState *state)
{
	...
	if(!(afterDraw + 3 == beforeDraw))
	{
		printf("Expected deck count %d, actual %d\n", beforeDraw-3, afterDraw);
	}
}
...
void testVPoints(struct gameState *state)
{
	...
	for(i = 0; i < state->numPlayers-1; i++)
	{
		if(!(beforeVictoryPoints[i] == scoreFor(i, state)))
		{
			printf("\nVictory Points Incorrect for player %d \n",i+1);
		}
		printf(".");
	}
	printf("\n");
...
void testHandPos(struct gameState *state, int handpos,int player,int cardsInHand)
{
	...
	if(!(cardsNowInHand == cardsShouldBeInHand))
	{
		printf("Inccorrect Cards in Hand, %d instead of %d", cardsNowInHand, cardsShouldBeInHand);
	}
return;
...
void testHandsSize(struct gameState *state, int cardsInHand, int player)
{
	...
	cardEffect( smithy , choice1, choice2, choice3, state, 0, &bonus);
	if(!(cardsShouldBeInHand == state->handCount[player])){
		printf("Incorrect Cards in Hand, %d instead of %d", state->handCount[player], cardsShouldBeInHand);
	}
