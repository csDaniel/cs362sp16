...
//3. It gives the player 2 additional actions. 
...
int main () {
  ...
  printf("Initial Num of Actions: %i\n", actual.numActions);


  //Set up expected results.
  expected.deckCount[player] = actual.deckCount[player] - 1;
  expected.numActions = actual.numActions + 2;

  int handpos = 0;

  if( playVillage(&actual, handpos) == 0){
  ...
  printf("Expected Deck Count: %i Actual Deck Count: %i\n", expected.deckCount[player], actual.deckCount[player]); 
 //+ 1 card from the deck -1 village card. Hand Count should be the same.
  ...
  printf("Expected num of Actions: %i Actual num of actions: %i\n", expected.numActions, actual.numActions);
