/******************************************************************************
* ASSIGNMENT 5 - randomtestadventurer.c
* AUTHOR: Nickolas A. Williams
* CLASS: CS362
* DATE: 05/22/2016
* NOTES: random testing Wesley Jinks's refactored playAdventurerCard() function
...
* TESTING tasks for playAdventurerCard():
...
int main() 
{
	...
	for (test = 0; test < NUM_TESTS; test++)
	{
		...
		//set StartState whoseTurn to the playing player for this test
		StartState.whoseTurn = player;
		//save gamestate for comparison
		memcpy(&TestState, &StartState, sizeof(struct gameState));
		//call playAdventurer()
		//playAdventurer(&TestState, player, handPos); --old call
		playAdventurerCard(&TestState); //adjusted call
